...
4. **Automated Previews & Zero-Downtime Launch:**
   - Reviewers test PRs via automated preview links generated by `LABEIM/shared-ci-cd`.
...
| **Pull Request Reviewers** | PR Comments in GitHub | **Automated Preview Deployments** posted directly by `LABEIM/shared-ci-cd` (`https://<hash>.smart-campus.pages.dev`). |
...
4. Developers build on `develop`. Every PR creates preview URLs via `LABEIM/shared-ci-cd`. On launch day, merge `develop` into `main`.
